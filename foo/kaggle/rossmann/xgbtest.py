...
def build_features(features, data):
    ...
    mappings = {'0': 0.0, 'a': 1.0, 'b': 2.0, 'c': 3.0, 'd': 4.0}
    data['StoreType'] = data['StoreType'].map(lambda x: ord(x) - ord('a'))
    data['Assortment'] = data['Assortment'].map(lambda x: ord(x) - ord('a'))
    data['StateHoliday'] = data['StateHoliday'].map(lambda x: ord(x) - ord('a'))
